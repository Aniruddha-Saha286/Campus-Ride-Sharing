...
export default function PaymentRequests() {
  ...
  const counts = useMemo(() => {
    ...
    let owedByMeCount = 0;
    data.forEach((r) => {
      if (r.role === "requester") owedToMeCount++;
      else if (r.role === "payer") owedByMeCount++;
    });
    return { all: data.length, owedToMe: owedToMeCount, owedByMe: owedByMeCount };
  }, [data]);
  ...
  const filteredRequests = useMemo(() => {
    ...
    // Filter by role tab
    if (requestTab === "owedToMe") list = list.filter((x) => x.role === "requester");
    else if (requestTab === "owedByMe") list = list.filter((x) => x.role === "payer");
    ...
    if (filterSearch.trim()) {
      ...
      list = list.filter((x) => {
        const name = (x.counterpart?.name || "").toLowerCase();
        const desc = (x.description || "").toLowerCase();
        const code = (x.requestCode || "").toLowerCase();
    ...
    return [...list].sort((a, b) => {
      ...
      if (sortBy === "amount_desc") return (b.amountDue || 0) - (a.amountDue || 0);
      if (sortBy === "amount_asc") return (a.amountDue || 0) - (b.amountDue || 0);
      if (sortBy === "due_date") {
  ...
  }, [data, requestTab, statusFilter, filterSearch, sortBy]);
      ...
      <div className="mx-auto w-full max-w-4xl">
        ...
        {data.length === 0 ? (
          ...
          <div className="space-y-6">
            {/* SORT & FILTER TOOLBAR */}
            <div className="flex flex-col gap-3 rounded-2xl border border-slate-100 bg-white p-4 shadow-card">
              <div className="flex flex-wrap items-center justify-between gap-3">
                {/* Role Tabs */}
                <div className="flex flex-wrap items-center gap-1.5">
                  {[
                    { key: "all", label: "All Requests", count: counts.all },
                    { key: "owedToMe", label: "To be paid to me", count: counts.owedToMe },
                    { key: "owedByMe", label: "I need to pay", count: counts.owedByMe },
                  ].map((tab) => (
                    <button
                      key={tab.key}
                      type="button"
              ...
              <div className="flex flex-wrap items-center justify-between gap-3 border-t border-slate-100 pt-3">
                ...
                <div className="relative min-w-[150px] flex-1 sm:w-56 sm:flex-initial">
                  ...
                  <input
                    ...
                    placeholder="Search requests / names..."
            ...
            {filteredRequests.length === 0 ? (
              <div className="flex flex-col items-center justify-center rounded-2xl border border-dashed border-slate-200 bg-white px-6 py-10 text-center shadow-card">
                ...
                <p className="mt-2 text-sm font-semibold text-slate-700">No matching payment requests</p>
                <p className="mt-1 text-xs text-slate-400">
                  {filterSearch ? `No requests matched "${filterSearch}".` : "No requests found for this filter."}
                </p>
                <button
                  type="button"
                  onClick={() => {
              ...
              <div className="space-y-4">
                {filteredRequests.map(requestCard)}
              </div>
            )}
          </div>
        )}
